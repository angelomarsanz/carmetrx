    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.models.length > 0) {
                $('.' + addedModel).append(
                    $('<option selected disabled></option>').val('').html('Seleccionar Modelo')
                );
                $('.model').show();
                ...
                $('.' + addedModel).find('option').remove().end().append(
                    $('<option selected ></option>').val('').html('No Model Found')
                );
            }
            // Reset version
            $('.version_id').empty().append(
                $('<option selected disabled></option>').val('').html('Seleccionar Versión')
            );
            $('.request-loader').removeClass('show');
        }
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.versions.length > 0) {
                ...
                $('.' + addedVersion).find('option').remove().end().append(
                    $('<option selected ></option>').val('').html('No Version Found')
                );
            }
            $('.request-loader').removeClass('show');
        }
